...
# Used as a flag to allow certain operations in the Node role
method is-element { True }
...
method index-of ($find) {
    return $_ if @!nodes[$_] ~~ $find for ^(@!nodes.elems);
    return False;
}
...
method !craft-new (Str $name, %attribs, @contents) {
    ...
    for @contents -> $what {
        ...
        if    $what ~~ XML::Node { $new.append($what)                           }
        elsif $what ~~ Capture   { $new.append(self.craft(|$what))              }
        elsif $what ~~ Str       { $new.append(XML::Text.new(:text($what)))     }
        elsif $what.can('Str')   { $new.append(XML::Text.new(:text($what.Str))) }
    }
    return $new;
  }
...
method parse-node ($node, $mother?) {
    ...
    for $node<attribute><> -> $a {
        my $an = $a<name>.Str;
        my $av = $a<value>.Str; # // ''; old <char>.list>>.ast.join // ''
        %attribs{$an} = $av;
    }

    my $parent = XML::Element.new(:$name, :%attribs);

    $parent.parent = $mother if $mother;
    ...
    for $node<child><> -> $c {
        ...
        @nodes.push: $child if defined $child;
    }
    $parent.nodes = @nodes;
    return $parent;
}
...
method elements (*%query) {
    my $recurse = 0;
    my $nest    = %query<NEST>    // False;
    my $single  = %query<SINGLE>  // False;
    my $object  = %query<OBJECT>  // False;
    my $byindex = %query<BYINDEX> // False;
    my @elements;
    my $nodepos = 0;

    $recurse = $_ eq True ?? Inf !! $_ with %query<RECURSE>;
        ...
        if $node.is-element {
            ...
            for %query.kv -> $key, Mu $val {
                if $key eq 'RECURSE' | 'NEST' | 'SINGLE' | 'OBJECT' | 'BYINDEX' {
                    ...
                    my $want-atpos;
                    my $pos;
                    my $last = @!nodes.grep(*.is-element).end;
...
method nsPrefix ($uri) {
    ...
    return $.parent.is-element ?? $.parent.nsPrefix($uri) !! Nil;
}
...
method nsURI ($prefix?) {
    ...
    return $.parent.is-element ?? $.parent.nsURI($prefix) !! Nil;
}
...
method AT-KEY ($offset) is rw {
    my $self = self;
    Proxy.new(
        FETCH => method        { $self.attribs{$offset}   },
        STORE => method ($val) { $self.set($offset, $val) }
    )
}
